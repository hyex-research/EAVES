...
- **Coefficient c**: back-solved as c = V_cap/A_cap^b from catalog capacity and a multi-feature linear regression that predicts log A_cap from seven log-space features: `capacity_mcm`, `dam_height_m`, `spillway_height_m`, `valley_ratio`, `channel_slope`, `mean_catchment_slope`, `upstream_area_km2`. Any feature missing for a given dam is imputed with the training-set median so the regression always returns a finite value.
