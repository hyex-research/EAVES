"""End-to-end regression check: run the 9-dam fixture and compare every
...
def test_fixture_outputs_match_golden(fixture_output: Path, golden_hashes: dict):
    ...
    if missing or mismatches:
        ...
        msg.append(
            ...
            "  pytest -m slow  # rebuilds tests/fixture/output/\n"
            "  python -c 'import hashlib,json; from pathlib import Path; "
            "out=Path(\"tests/fixture/output/1_results_csv\"); "
            "h={str(p.relative_to(\"tests/fixture/output\")): hashlib.sha256(p.read_bytes()).hexdigest() "
            "for p in sorted(out.rglob(\"*.csv\"))}; "
            "open(\"tests/golden_hashes.json\",\"w\").write(json.dumps(h,indent=2,sort_keys=True))'"
        )
        pytest.fail("\n\n".join(msg))
