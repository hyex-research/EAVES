...
def baish_validation(summary, output_dir):
    ...
    with mpl.rc_context(_ANALYSIS_RC):
        ...
        ax.set_title("Baish: Area\u2013Volume comparison")
        ax.legend()
        ...
        ax.set_title("Baish: Elevation\u2013Area comparison")
        ax.legend()
        ax.grid(True, ls="--", alpha=0.3, lw=0.4)

        plt.savefig(os.path.join(output_dir, "baish_validation.png"),
...
def grdl_comparison(summary_df, output_dir):
    ...
    with mpl.rc_context(_ANALYSIS_RC):
        ...
        plt.savefig(os.path.join(output_dir, "grdl_comparison.png"),
                    dpi=300, bbox_inches="tight")
        plt.close(fig)
